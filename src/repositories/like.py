...
class LikeRepository(BaseRepository):
    ...
    async def get_like_by_post_author_id(self, post_id: int, author_id: int, *options) -> int:
        stmt = select(self.model).where(self.model.post_id == post_id, self.model.author_id == author_id).options(selectinload(self.model.author), selectinload(self.model.post))
        if options:
